...
// Renderer/program/mesh survivent aux re-renders pour que le 2e effet mette
// à jour les uniforms sans jamais reconstruire le contexte WebGL.
type GrainientCtx = {
...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      // container.getBoundingClientRect() renvoie déjà la taille affichée
      // (après le transform:scale() du Canvas éditeur) — renderer.setSize()
      // réécrit canvas.style.width/height en px sur cette base, ce qui
      // ferait re-appliquer le scale une seconde fois. On réimpose 100% pour
      // que le canvas suive la taille CSS de son conteneur, pas du viewport.
  ...
  useEffect(() => {
    ...
    // Repeint immédiatement : si la boucle RAF est en pause (onglet masqué,
    // conteneur hors champ), les nouveaux uniforms resteraient invisibles
    // jusqu'à la reprise de la boucle sans ce rendu forcé.
